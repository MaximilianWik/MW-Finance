...
import { categorizeBatch } from "@/lib/categorize-batch";
...
export async function runSync(
  ...
  try {
    ...
    if (inserted.length > 0) {
      ...
      const catStats = await categorizeBatch(catRows, { useGemini: opts.useGemini ?? false, onLog: push });
      const catParts = [
